...
const Input = () => {
  ...
  const $emailCheck = React.createRef()
  ...
  const emailValidation = (email) => {
    if (email.length === 0) {
      $emailCheck.current.classList.remove("text-green-500")
    }

    const emailRegex = /^[a-zA-Z0-9+-\_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/
    const isOk = emailRegex.test(email)
    if (isOk) {
      $emailCheck.current.classList.add("text-green-500")
  ...
  return (
    <div className="input-component w-1/2 h-36 bg-white-400 border-2 border-black rounded-lg flex-row mt-3 bg-slate-100">
      <form className="p-3 w-full h-full">
        <div className="email-box flex flex-col h-1/2 relative">
          <div
            ref={$errorMessage}
            className="absolute left-20 -top-1 text-sm text-red-700"
            style={{ display: "none" }}
          >
            invalid email
          </div>
